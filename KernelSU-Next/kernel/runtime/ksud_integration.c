				...
				loff_t *pos)
{
	...
	/*
	 * Append the KernelSU rc block once we observe EOF. Older readers loop
	 * until read() returns 0; newer init (Android 16/17) reads the whole
	 * regular file in a single short read and stops without a trailing
