...
static struct sidtab *ksu_get_sidtab(void)
{
#ifdef SELINUX_POLICY_INSTEAD_SELINUX_SS
    return selinux_state.policy->sidtab;
#elif defined(KSU_COMPAT_USE_SELINUX_STATE)
    return selinux_state.ss->sidtab;
#else
    return &sidtab;
#endif
}
...
u32 ksu_compute_av_delta_bits(u32 ssid, u32 tsid, u16 tclass)
{
    ...
    sc = sidtab_search(sidtab, ssid);
    tc = sidtab_search(sidtab, tsid);
    if (!sc || !tc)
        goto out;
    if (sc->type == 0 || tc->type == 0 ||
        sc->type > db->p_types.nprim || tc->type > db->p_types.nprim)
        goto out;

    sattr = &db->type_attr_map_array[sc->type - 1];
    tattr = &db->type_attr_map_array[tc->type - 1];

    ebitmap_for_each_positive_bit(sattr, snode, i)
    {
        ebitmap_for_each_positive_bit(tattr, tnode, j)
        {
            bits |= ksu_avc_delta_lookup(i + 1, j + 1, tclass);
        }
    }
out:
