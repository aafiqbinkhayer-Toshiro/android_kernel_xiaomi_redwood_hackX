...
static void ksu_avc_delta_add(u32 stype, u32 ttype, u16 tclass, u32 bits)
{
    ...
    struct avtab_node *n = avtab_search_node(&ksu_delta_avtab, &key);
    if (n) {
        n->datum.u.data |= bits;
        return;
    }
...
u32 ksu_avc_delta_lookup(u32 stype, u32 ttype, u16 tclass)
{
    ...
    n = avtab_search_node(&ksu_delta_avtab, &key);
    for (; n; n = avtab_search_node_next(n, AVTAB_ALLOWED))
        bits |= n->datum.u.data;
    return bits;
}
                         ...
                         struct perm_datum *perm, int effect, bool invert)
{
    if (src == NULL) {
        ...
        if (ksu_avc_delta_ready && !invert &&
            ...
            tgt->value && tgt->value <= ksu_avc_genuine_ntypes) {
            u32 ksu_added = node->datum.u.data & ~ksu_before;
            if (ksu_added)
