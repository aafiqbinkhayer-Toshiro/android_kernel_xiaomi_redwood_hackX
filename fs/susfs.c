...
// ksu_cred = KOWSU's cred pinned to the KSU_DOMAIN_CONTEXT domain (setup_ksu_cred()).
// The re-flag runs from a kworker (u:r:kernel:s0), which SELinux denies path
// traversal into app/data dirs (kern_path -> -EACCES). Override to ksu_cred for
// the lookups, exactly as upstream susfs4ksu and KOWSU's own umount path do.
...
/* No default uname spoof: my_uname stays zeroed until (and unless) the
 * ksu_susfs CLI arms it, so uname(2) and /proc/version report the real
 * UTS_RELEASE/UTS_VERSION, which is what /proc/sys/kernel/osrelease has always
 * reported.
 * The key TYPE must stay static_key_false to match `extern struct static_key_false
 * susfs_set_uname_key_true;` in kernel/sys.c (a TRUE define breaks static_branch). It is left
 * OFF at boot by susfs_init()'s static_branch_disable below. */
static struct st_susfs_uname my_uname;
DEFINE_STATIC_KEY_FALSE(susfs_set_uname_key_true);
...
/* default OFF: susfs_init() not yet wired; command channel re-enables on SET_CMDLINE_OR_BOOTCONFIG */
DEFINE_STATIC_KEY_FALSE(susfs_set_fake_cmdline_or_bootconfig_key_true);
...
void susfs_add_open_redirect(void __user **user_info) {
	...
	if (is_first_dup_found) {
		...
		mutex_unlock(&susfs_mutex_lock_open_redirect);
		// upstream a74b1eb: OPEN_REDIRECT hlist is SRCU-protected, so sync on its own
		// SRCU domain (global synchronize_rcu never waits for an SRCU read-side section) -> UAF fix
...
/* default OFF: susfs_init() not yet wired; command channel re-enables on SET_AVC_LOG_SPOOFING */
...
static int susfs_sdcard_monitor_fn(void *data)
{
	...
	setup_selinux(KSU_DOMAIN_CONTEXT, cred);
	commit_creds(cred);
