...
show_map_vma(struct seq_file *m, struct vm_area_struct *vma)
{
	...
	if (file) {
		...
		if (SUSFS_IS_INODE_SUS_MAP(inode)) {
			/*
			 * The kernel uses a fixed-width show_vma_header_prefix() for the
			 * maps formatter. Emit the anonymized sus_map line through that
			 * same helper (zeroed dev/ino, no name) so it is byte-identical in
			 * shape to every other line.
			 */
			dev = 0;
			ino = 0;
			pgoff = ((loff_t)vma->vm_pgoff) << PAGE_SHIFT;
